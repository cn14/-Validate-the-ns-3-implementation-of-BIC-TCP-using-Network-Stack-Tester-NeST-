...
std::string dir = "TcpBic_results/";
int check = mkdir("TcpBic_results",0777); 
...
Ptr<Socket> uniFlow(Address sinkAddress, 
	 ...
	 if((tcpVariant.compare("TcpBic")) == 0) {
...
//	AsciiTraceHelper asciiTraceHelper;
	...
	Ptr<Socket> ns3TcpSocket1 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(0), port), port,tcpVariant2, senders.Get(0), receivers.Get(0), oneFlowStart,FlowEndtime, packetSize, numPackets*1000, transferSpeed, oneFlowStart,FlowEndtime);
...
//	std::cout << "TCP Westwood from H2 to H5" << std::endl;
	...
	Ptr<Socket> ns3TcpSocket2 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(1), port), port,tcpVariant2, senders.Get(1), receivers.Get(1),oneFlowStart,FlowEndtime, packetSize, numPackets*1000, transferSpeed,oneFlowStart,FlowEndtime);
		...
		*streamTP->GetStream()  << "  Throughput: " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024/1024  << " Mbps\n";	
		...
		if(t.sourceAddress == "10.1.0.1") {
			if(mapDrop.find(1)==mapDrop.end())
				mapDrop[1] = 0;
			*stream1PD->GetStream() << "TcpBIc Flow " << i->first  << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
			...
			*stream1PD->GetStream() << "Max throughput: " << mapMaxThroughput["/NodeList/5/$ns3::Ipv4L3Protocol/Rx"] << std::endl;
			...
			if(mapDrop.find(2)==mapDrop.end())
				mapDrop[2] = 0;
			*stream2PD->GetStream() << "TcpBIc Flow " << i->first  << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
